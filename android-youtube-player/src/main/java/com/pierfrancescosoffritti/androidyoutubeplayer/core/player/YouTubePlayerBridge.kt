...
class YouTubePlayerBridge(
    ...
    @JavascriptInterface
    fun sendAvailableQualityLevels(levels: String) {
        val qualityLevels =
            JSONArray(levels).let { json ->
                List(json.length()) { index ->
                    parsePlaybackQuality(json.getString(index))
                }
            }

        mainThreadHandler.post {
            youTubePlayerOwner.listeners.forEach {
                it.onAvailableQualityLevels(
    ...
    @JavascriptInterface
    fun sendCaptionAvailable(available: Boolean) {

        mainThreadHandler.post {
            youTubePlayerOwner.listeners.forEach {
                it.onCaptionAvailable(
                    youTubePlayerOwner.getInstance(),
                    available,
                )
            }
